...
CREATE TABLE IF NOT EXISTS raw_ais (
    ...
    length_m    DOUBLE,
    ais_status  VARCHAR,
    is_water    BOOLEAN,             -- true=over sea/EEZ, false=over land
                                     -- (computed by backend.sensors._water_mask)
...
CREATE TABLE IF NOT EXISTS raw_sar (
    ...
    is_water        BOOLEAN,             -- true=detection over sea, false=over land
                                         -- (CFAR fires on bright reflectors anywhere;
                                         --  land-positives are filtered downstream)
    payload_json    VARCHAR,
    PRIMARY KEY (cache_key, event_id)
);
...
-- Pre-baked canonical brief per AoI. When present, the /api/aoi/{id}/brief
-- endpoint serves this instead of running the 4-agent pipeline live. Used
-- to take latency off the demo's critical path: cold pipeline ~12-15s,
-- cached ~50ms. Generated by scripts/pre_cache_briefs.py.
CREATE TABLE IF NOT EXISTS aoi_canonical_brief (
    aoi_id        VARCHAR PRIMARY KEY,
    brief_json    VARCHAR NOT NULL,    -- the full {id, watch_id, sections:[...], …} payload
    generated_at  TIMESTAMP NOT NULL,
    notes         VARCHAR              -- free-text — e.g. "hand-tuned for pitch demo"
);
